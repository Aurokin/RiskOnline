...
socket.on("createRoom", function(name){
	if(people[socket.id].inroom){
		socket.emit("update", "You are in a room. Please leave it before creating another room.");
	}
		...
		socket.emit("update", "You've already created a room.");
	}
});
...
socket.on("removeRoom", function(id){
	...
	if (socket.id === room.owner){
		...
		socket.emit("update", "Only the owner can remove the room.");
	}
});
...
socket.on("joinRoom", function(id){
	if(typeof people[socket.id] !== "undefined"){
		var room = rooms[id];
		if (socket.id === room.owner){
			socket.emit("update", "You are the owner of this room and you have already joined.");
		}
		else{
			if(_.contains((room.people), socket.id)){
				socket.emit("update", "You ahve already joined this room.");
			}
			else{
				if(people[socket.id].inroom !== null){
					socket.emit("update", "You are already in a room ("+rooms[people[socket.id].inroom].name+")");
				}
				else{
					room.addPerson(socket.id);
					people[socket.id].inroom = id;
